...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(isset($_POST['rev-flag']) && ($_POST['rev-flag'] == '000')){

        if(isset($_POST['rev-comment_text']) && isset($_POST['rev-post_no'])){

            $comment_text = $Fun_call->validate($_POST['rev-comment_text']);
            $post_no = $Fun_call->validate($_POST['rev-post_no']);
            ...
            if(!empty(trim($comment_text)) && (strlen($comment_text) <= 8000) && !empty(trim($post_no)) && is_numeric($post_no)){
                ...
                $insert = $Fun_call->insert('review_comments', $field_val);
    ...
    elseif(isset($_POST['rev-R_flag']) && ($_POST['rev-R_flag'] == '111')){

        if(isset($_POST['rev-replay_text']) && isset($_POST['rev-user_no']) && isset($_POST['rev-replay_no'])){

            $replay_text = $Fun_call->validate($_POST['rev-replay_text']);
            $user_no = $Fun_call->validate($_POST['rev-user_no']);
            $replay_no = $Fun_call->validate($_POST['rev-replay_no']);
            ...
            if(!empty(trim($replay_text)) && (strlen($replay_text) <= 8000) && !empty(trim($user_no)) && is_numeric($user_no) && !empty(trim($replay_no)) && is_numeric($replay_no)){
                ...
                $insert_rep = $Fun_call->insert('review_rep_comments', $field_val);
